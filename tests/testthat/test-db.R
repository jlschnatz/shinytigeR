...
make_item_db <- function(path) {
  ...
  DBI::dbWriteTable(con, "item_db", data.frame(
    ...
    answeroption_03 = NA, answeroption_04 = NA,
    answeroption_05 = NA, answeroption_06 = NA,
    if_answeroption_01 = c("F1", "F1", "F1"),
    if_answeroption_02 = c("F2", "F2", "F2"),
    if_answeroption_03 = NA, if_answeroption_04 = NA,
    if_answeroption_05 = NA, if_answeroption_06 = NA,
...
make_response_row <- function(user_id = "test_user", item_id = 1L,
                               learning_area = "Regression",
                               bool_correct = TRUE, skipped = FALSE) {
  data.frame(
    id_user         = user_id,
    id_session      = "sess1",
...
test_that("db_get_items returns a data.frame with expected columns", {
  ...
  expect_true(all(c("id_item", "learning_area", "type_item",
                     "irt_discr", "irt_diff") %in% names(df)))
})
