...
│   ├── run_app.R          # run_app() — the single exported entry point
...
│   ├── Dockerfile         # auto-generated by build.R — do not edit by hand
...
Static files are served via `shiny::addResourcePath()` in `inst/app/app.R`:
...
`normalizePath()` / `system.file()` is used to resolve paths correctly whether running from source or from an installed package. **Do not use relative paths** for resource registration.
