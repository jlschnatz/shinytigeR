...
mod_practice_server <- function(id, data_item, practice_ids, credentials, write_trigger) {
  moduleServer(id, function(input, output, session) {
    ...
    output$item_stimulus <- renderUI({
      ...
      item <- as.list(item[1, ])
      tagList(
        div(class = "stimulus mb-2",
          if (!is.na(item$stimulus_text) && nzchar(trimws(item$stimulus_text)))
            div(class = "stimulus-text", shiny::HTML(render_md(item$stimulus_text))),
          if (!is.na(item$stimulus_image) && nzchar(item$stimulus_image))
            div(class = "text-center my-3",
              tags$img(src = item$stimulus_image, class = "img-fluid stimulus-img"))
        ),
    ...
    output$item_answers <- renderUI({
      ...
      answers_ui <- if (!state$checked) {
        radio_opts <- lapply(seq_along(choices), function(i) {
          ...
          if (item$type_answer == "image" && is_img_path(choices[i])) {
            div(class = "d-flex align-items-center mb-3",
              tags$input(type = "radio", class = "form-check-input me-2",
                         name = ns("answer"), id = id_i, value = i,
                         style = "width:1.2em;height:1.2em;cursor:pointer;"),
              tags$label(`for` = id_i, class = "label-answer",
            ...
            div(class = "answer-option mb-2",
              ...
              tags$label(`for` = id_i, class = "ms-2 answer-label",
        ...
        fb_text   <- if (state$answer_id <= length(feedbacks)) feedbacks[[state$answer_id]] else ""
        suffix    <- if (item$type_answer == "image") "img" else "txt"
        hi_class  <- paste0(result, "_answer_", suffix)
        color_var <- switch(result,
          correct   = "var(--tiger-correct)",
          incorrect = "var(--tiger-incorrect)",
          skip      = "var(--tiger-skip)"
        )
        icon_name <- switch(result,
          correct = "check-circle-fill", incorrect = "x-circle-fill",
          skip    = "skip-forward-fill"
        )
        title_str <- switch(result,
          correct = "Richtig!", incorrect = "Leider falsch", skip = "Übersprungen"
        )
        ...
        tagList(
          lapply(seq_along(choices), function(i) {
            ...
            id_i      <- paste0(ns("answer"), "_", i)
            radio_cls <- paste("form-check-input",
                               if (is_sel) paste0("radio-result-", result) else "")
            if (item$type_answer == "image" && is_img_path(choices[i])) {
              div(class = paste("d-flex align-items-center mb-3", if (is_sel) hi_class else ""),
                tags$input(type = "radio", class = paste(radio_cls, "me-2"),
                           name = paste0(ns("answer"), "_done"), id = id_i,
                           disabled = NA, checked = if (is_sel) NA else NULL,
              ...
              div(class = paste("answer-option mb-2", if (is_sel) hi_class else ""),
                tags$input(type = "radio", class = radio_cls,
                           name = paste0(ns("answer"), "_done"), id = id_i,
                           disabled = NA, checked = if (is_sel) NA else NULL,
          ...
          if (nzchar(fb_text))
            div(class = "feedback-card mt-4",
              ...
              div(class = "feedback-header",
                ...
                bsicons::bs_icon(icon_name), tags$b(title_str)
              ),
              div(class = "feedback-body mt-1",
                shiny::HTML(render_md(fb_text)))
            )
        )
      }
    ...
    observeEvent(list(state$pos, practice_ids()), {
      state$checked   <- FALSE
      state$answer_id <- NULL
    ...
    observeEvent(input$check, {
      ...
      tryCatch({
        ...
        showNotification(
          paste("Antwort konnte nicht gespeichert werden:", conditionMessage(e)),
