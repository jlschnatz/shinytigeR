...
competency_label <- function(theta, n_unique) {
  if (n_unique == 0L || is.na(theta))
    return(list(label = "Keine Daten", bg = "#e9ecef", fg = "#6c757d", level = 0L))
  if      (theta >  1.0) list(label = "Stark",           bg = "#d1ecf1", fg = "#00618f", level = 4L)
  else if (theta >  0.0) list(label = "Gut entwickelt",  bg = "#d4edda", fg = "#1a6b3c", level = 3L)
  else if (theta > -0.5) list(label = "Entwickelt sich", bg = "#fff3cd", fg = "#856404", level = 2L)
  else                   list(label = "Übungsbedarf",     bg = "#fce4ec", fg = "#D81B60", level = 1L)
}

evidence_label <- function(n_unique) {
  if      (n_unique >= 8L) list(label = "Hoch",    dots = 3L, color = "#198754")
  else if (n_unique >= 3L) list(label = "Mittel",  dots = 2L, color = "#fd7e14")
  else if (n_unique >= 1L) list(label = "Niedrig", dots = 1L, color = "#dc3545")
  else                     list(label = "–",        dots = 0L, color = "#adb5bd")
}
...
pct_bar <- function(pct, color = PRIMARY_COLOR) {
  if (is.na(pct)) return(tags$span(class = "text-muted small", "–"))
  tags$div(
...
recommend_next <- function(comp, n_unique_vec) {
  recs <- lapply(LEARNING_AREA_LEVELS, function(area) {
    ...
    if (is.na(n) || n == 0L)
      list(area = area, priority = 10L,
           reason = "Noch keine Aufgaben bearbeitet")
    else if (n < 3L)
      list(area = area, priority = 20L,
           reason = sprintf("Zu wenig Daten (%d Aufgabe%s) — mehr bearbeiten",
                            n, if (n == 1L) "" else "n"))
    else if (!is.na(theta) && theta < -0.5)
      list(area = area, priority = as.integer(30L + round(theta * -10L)),
...
mod_dashboard_server <- function(id, data_item, credentials, write_trigger) {
  moduleServer(id, function(input, output, session) {
    ...
    competency <- reactive({
      fa <- first_attempts()
      if (nrow(fa) == 0L) return(NULL)
      fa$learning_area <- factor(fa$learning_area, levels = LEARNING_AREA_LEVELS)
    ...
    output$dash_content <- renderUI({
      ...
      pct_correct <- if (n_unique > 0L) round(100 * n_correct / n_unique) else 0L
      ...
      n_unique_area <- setNames(
        vapply(LEARNING_AREA_LEVELS, function(a)
          sum(fa$learning_area == a, na.rm = TRUE), integer(1)),
      ...
      area_type_acc <- lapply(LEARNING_AREA_LEVELS, function(a) {
        rows <- merge(
          fa[!is.na(fa$learning_area) & fa$learning_area == a & !is.na(fa$bool_correct), ],
          data_item[, c("id_item", "type_item")],
          by = "id_item", all.x = TRUE
        )
        content <- rows[!is.na(rows$type_item) & rows$type_item == "content", ]
        coding  <- rows[!is.na(rows$type_item) & rows$type_item == "coding",  ]
        list(
          content_acc = if (nrow(content) > 0L) mean(content$bool_correct, na.rm=TRUE) else NA_real_,
          coding_acc  = if (nrow(coding)  > 0L) mean(coding$bool_correct,  na.rm=TRUE) else NA_real_,
      ...
      bloom_acc  <- setNames(
        vapply(bloom_lvls, function(b) {
          rows <- merge(fa[!is.na(fa$bool_correct), ],
                        data_item[, c("id_item", "bloom_taxonomy")],
                        by = "id_item", all.x = TRUE)
          sub  <- rows[!is.na(rows$bloom_taxonomy) & rows$bloom_taxonomy == b, ]
          if (nrow(sub) == 0L) NA_real_ else mean(sub$bool_correct, na.rm = TRUE)
        }, numeric(1)),
        bloom_lvls
      )

      # ── Recommendations ───────────────────────────────────────────────────────
      recs <- if (!is.null(comp)) recommend_next(comp, n_unique_area) else list()
      ...
      tagList(
        ...
        div(class = "row g-3 mb-4",
          div(class = "col-sm-4",
            bslib::value_box(
              title = "Geübt an", value = sprintf("%d Tag%s", n_days, if(n_days==1)"" else "en"),
              showcase = bsicons::bs_icon("calendar-check"), theme = "primary"
            )
          ),
          div(class = "col-sm-4",
            bslib::value_box(
              title = "Einmalige Aufgaben",  value = n_unique,
            ...
            bslib::card(
              ...
              bslib::card_header(
                div(class = "d-flex align-items-center gap-2",
                  bsicons::bs_icon("map"), tags$b("Kompetenzprofil"),
                  tags$span(class = "text-muted small ms-1",
                    "(nur Erstversuche · ●●● = hohe Evidenz)")
                )
              ),
              bslib::card_body(class = "p-0",
                tags$table(class = "table table-sm table-hover mb-0 dashboard-comp-table",
                  tags$thead(
                    tags$tr(
                  ...
                  tags$tbody(
                    lapply(LEARNING_AREA_LEVELS, function(area) {
                      ...
                      theta <- if (!is.null(comp)) comp$theta[comp$learning_area == area] else NA_real_
                      cl    <- competency_label(theta, n_a)
                      at    <- area_type_acc[[area]]

                      short_area <- names(LEARNING_AREA_LABELS)[LEARNING_AREA_LABELS == area]
                      if (length(short_area) == 0L) short_area <- area
                      ...
                      tags$tr(
                        ...
                        tags$td(
                          tags$span(
                            ...
                            style = sprintf("background:%s;color:%s;font-weight:500;",
                                            cl$bg, cl$fg),
                            cl$label
                          )
                        ),
              ...
              bslib::card_body(
                if (length(recs) == 0L) {
                  ...
                  tagList(lapply(seq_along(recs), function(i) {
                    r      <- recs[[i]]
                    short  <- names(LEARNING_AREA_LABELS)[LEARNING_AREA_LABELS == r$area]
                    if (length(short) == 0L) short <- r$area
                    div(class = "rec-item d-flex gap-3 mb-3",
                      div(class = "rec-num", i),
                      div(
        ...
        div(class = "row g-3 mb-4",
          ...
          div(class = "col-md-5",
            bslib::card(
              ...
              bslib::card_header(
                div(class = "d-flex align-items-center gap-2",
                  bsicons::bs_icon("layers"), tags$b("Aufgabentyp (Bloom)"))
              ),
              bslib::card_body(
                  ...
                  div(class = "mb-3",
                    div(class = "d-flex justify-content-between mb-1",
                      ...
                      tags$span(class = "small text-muted",
                        if (is.na(acc)) "Keine Daten"
                        else sprintf("%d%%", round(acc * 100)))
                    ),
                    if (!is.na(acc))
                      div(class = "progress", style = "height:8px;",
                        div(class = "progress-bar",
                            style = sprintf("width:%d%%;background:%s;",
                                            round(acc*100), PRIMARY_COLOR)))
                    else
                      div(class = "progress", style = "height:8px;",
                        div(class = "progress-bar",
          ...
          div(class = "col-md-7",
            bslib::card(
              ...
              bslib::card_header(
                div(class = "d-flex align-items-center gap-2",
                  bsicons::bs_icon("calendar3"), tags$b("Übungsverhalten"))
              ),
              bslib::card_body(
                div(class = "row g-2 mb-3",
                  div(class = "col-6",
                    div(class = "practice-stat",
                      ...
                      div(class = "practice-stat-lbl",
                          sprintf("Aktive Tag%s gesamt", if(n_days==1)"" else "e"))
                    )
                  ),
                  div(class = "col-6",
                    div(class = "practice-stat",
                      ...
                      div(class = "practice-stat-lbl",
                          sprintf("Tag%s diese Woche", if(n_this_week==1)"" else "e"))
                    )
                  ),
                  div(class = "col-6",
                    div(class = "practice-stat",
                ...
                if (n_repeated > 0L) {
                  ...
                  div(class = "mt-2",
                    div(class = "d-flex justify-content-between small text-muted mb-1",
                      span("Neue Aufgaben"), span(sprintf("%d%%", pct_new))),
                    div(class = "progress", style = "height:8px;",
                      div(class = "progress-bar",
                          style = sprintf("width:%d%%;background:#285f8a;", pct_new)),
                      div(class = "progress-bar",
                          style = sprintf("width:%d%%;background:#b0c9dc;", 100-pct_new))
                    ),
    ...
    output$learning_curve_plot <- renderPlot({
      ...
      ud$area_short <- ifelse(
        ...
        names(LEARNING_AREA_LABELS)[match(ud$learning_area, LEARNING_AREA_LABELS)],
        ud$learning_area
      )

      ggplot2::ggplot(ud, ggplot2::aes(x = item_num, y = roll_acc)) +
        ggplot2::geom_line(colour = PRIMARY_COLOR, linewidth = 1) +
        ggplot2::geom_point(ggplot2::aes(colour = area_short),
                            size = 2, alpha = 0.6, show.legend = TRUE) +
