...
protect_math_delimiters <- function(text) {
  ...
  math_display <- regmatches(text, gregexpr("\\$\\$[\\s\\S]+?\\$\\$", text, perl = TRUE))[[1]]
  for (r in math_display)
    text <- sub(r, escape_md_meta(r), text, fixed = TRUE)

  math_inline  <- regmatches(text, gregexpr("\\$[^$\n]+?\\$", text, perl = TRUE))[[1]]
...
render_md <- function(text) {
  ...
  markdown::markdownToHTML(text = protect_math_delimiters(text), fragment.only = TRUE)
}
...
evaluate_answer <- function(item, answer_idx) {
  ...
  if      (answer_idx == item$answer_correct) "correct"
  else if (answer_idx == n)                   "skip"
  else                                        "incorrect"
}
...
is_img_path <- function(x) {
  !is.na(x) & grepl("\\.(?:png|jpg|jpeg|svg|gif)$", x, ignore.case = TRUE, perl = TRUE)
}
